...
# graph_traj = pgibbs.sample_trajectory_loglin(dataframe=df, n_particles=50, n_samples=500)
#
...
#
# top5 = graph_traj.empirical_distribution().mode(5)
# for graph, prob in top5:
...
graph_traj = pgibbs.sample_trajectory_ggm(dataframe=df, n_particles=50, n_samples=100)
...
#graph = dlib.gen_AR2_graph(10)
#graph.add_edges_from([(0, 1), (1, 2), (2, 3)])

levels = np.array([range(2)] * graph.order())
table = loglin.sample_prob_table(graph, levels, 1.0)

# check so that th marginal probabilities for the variables in the separator are consistent
# print table[(slice(2), 0, slice(2))].sum()
# print table[[slice(2), 1, slice(2)]].sum()
#
# print local_tables[frozenset([0, 1])][slice(2), 0].sum()
# print local_tables[frozenset([0, 1])][slice(2), 1].sum()
#
# print local_tables[frozenset([2, 1])][0, slice(2)].sum()
# print local_tables[frozenset([2, 1])][1, slice(2)].sum()
#
# #print local_tables[frozenset([3, 4])][slice(2), 0].sum()
# #print local_tables[frozenset([3, 4])][slice(2), 1].sum()
#
# print table[Ellipsis, 0].sum()
# print table[Ellipsis, 1].sum()
#
...
graph_traj.write_file("traj.json")
#plt.show()
